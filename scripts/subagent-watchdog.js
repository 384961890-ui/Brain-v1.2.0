...
const WATCHDOG_LOG = resolvePath(_cfg.paths.watchdog_log);
const MAX_RETRIES = _cfg.limits.max_retries;
const DEFAULT_TIMEOUT = _cfg.limits.default_timeout_ms; // 20分钟（爸爸特调）
...
function log(message) {
    ...
    try {
        ...
        console.error(`[watchdog] 日志写入失败: ${e.message}`);
        if (e.code === 'ENOENT') {
            try {
                fs.mkdirSync(path.dirname(WATCHDOG_LOG), { recursive: true });
                fs.appendFileSync(WATCHDOG_LOG, line);
            } catch (_) {}
        }
    }
...
async function execWithRetry(cmd, options = {}) {
    ...
    for (let i = 1; i <= maxRetries; i++) {
        ...
        try {
            ...
            const isTimeout = e.killed || e.signal === 'SIGTERM' || (e.status === null && !e.message.includes('ENOENT'));
            const isKilled = isTimeout;
...
function runSubagent(task, timeout = DEFAULT_TIMEOUT) {
    ...
    const tempFile = path.join(require('os').tmpdir(), `watchdog-task-${crypto.randomBytes(8).toString('hex')}.json`);
    tempFiles.add(tempFile);
    try {
        fs.writeFileSync(tempFile, taskJson);
        const result = execFileSync(
            'openclaw',
            ['sessions_spawn', '--runtime', 'subagent', '--task-file', tempFile],
