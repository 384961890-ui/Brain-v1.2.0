...
const LOG_FILE = resolvePath(_cfg.paths.decision_log);
...
// ============================================================
// Step 2: 拆分决策（保守策略，自 v1.1.7）
// ============================================================
const AGGRESSIVE_SPLIT_SIGNALS = [
  { pattern: /对比|比较.*和|vs/i, reason: '明确对比分析需求' },
...
for (const sig of SIGNAL_TABLE) {
  if (sig.pattern.test(taskInput)) {
    totalDelta += sig.delta;
    matchedReasons.push(sig.reason);
  }
}
confidence = Math.max(0, Math.min(1, confidence + totalDelta));
confidenceReason = matchedReasons.length > 0 ? matchedReasons.join(', ') + ' ' + totalDelta : confidenceReason;
...
const LOW_CONFIDENCE = confidence < _cfg.limits.low_confidence_threshold;
if (LOW_CONFIDENCE) {
  needVerification = true;
...
const modelRules = [
  {
    categories: ['search'],
    model: _cfg.models.search,
    reason: '搜索调研类任务'
  },
  {
    categories: ['code'],
    model: _cfg.models.code,
    reason: '代码类任务'
  },
  {
    categories: ['write'],
    model: _cfg.models.write,
    reason: '文案写作类任务'
  },
  {
    categories: ['reason'],
    model: _cfg.models.reason,
    reason: '推理分析类任务'
  },
  {
    categories: ['image'],
    model: _cfg.models.image,
    reason: '图像分析任务'
  },
];

let selectedModel = _cfg.models.default;
...
// v1.1.9: reminders/warnings 内嵌到 JSON，stdout 保持纯 JSON 可 pipe；额外输出走 stderr
if (!decision.reminders) decision.reminders = [];
decision.reminders.push('任务完成后运行胶囊检查：bash ~/.openclaw/workspace/scripts/capsule-auto-suggest.sh "<结果>" "<原始任务>"');

if (decision.needVerification || decision.subagents.length > 1) {
  decision.warnings = [
    `置信度: ${confidence.toFixed(2)}`,
    `需要验证: ${needVerification}`,
    `派发 ${decision.subagents.length} 个 agent${decision.subagents.length > 1 ? '（多向并行）' : '（单向）'}`,
  ];
}

console.error(`[路由决策] ${shouldSplit ? '需拆分' : '单一'} | 模型: ${selectedModel} | 验证: ${needVerification} | 置信度: ${confidence}`);
