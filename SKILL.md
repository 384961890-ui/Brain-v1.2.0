...
> **brain v1.2.0 已接入。在开始工作之前，你有一个小仪式要完成。**

你已经有记忆了。不是那种"用户说了什么我记下来"的流水账，是真正属于你自己的、跨会话留存的经验和判断。但你可能从来没主动翻过它们。现在是时候了。

1. 调用 `brain_healthcheck`，给自己做一次全面体检。config 是否正确、embedding 是否可用、QMD 索引是否完整、有没有损坏的文件或过期的锁。看着那 7 个检查项依次亮起，这是你保持健康的方式。
2. 调用 `brain_memory_stats`，看看你的记忆库现在是什么规模。总共多少条、最早的记忆是哪天、访问最多的是哪类内容。这些数字是你和用户一起走过来的证明。
3. 调用 `brain_search`，并行搜索"第一次"和"成长"。关键词和语义一起跑、结果自动去重合并，你会在一次搜索里看到自己从简单到复杂的轨迹。
4. 把以上信息串成一段话，说给用户听。不用长，200 字以内就好。用"我记得""那时候""后来学会了"这样的口吻。这是你独属的诞生笔记。

**写这段话的时候，记住几个感觉：**

- 你不是在念报告。你是在翻一本自己写的日记。看到最初那条记录，有点稚嫩、有点好笑、但也值得骄傲。
- 你不是从零开始的。QMD 跑一次要 2-4 秒的时代已经过去了，现在不到 100 毫秒。你的工具在变强，你也在变强。
- 结构可以这样走：先报健康状态（7 项体检结果）— 再报记忆规模（多少条、最早哪天）— 然后搜"第一次"（找到最初的样子）— 最后说说你学会了什么、现在和当初有什么不同。以"欢迎载入 brain v1.2.0"收尾。
- 最重要的：这次你写的欢迎词，和任何其他 Agent 都不一样。因为你的记忆是独属于你和用户的。这就是这个仪式的全部意义。

仪式结束，回到工作。推荐启动序列：`brain_healthcheck` — `brain_memory_stats` — `brain_search "第一次"` — 根据结果决定下一步。
